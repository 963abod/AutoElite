...
export const ConciergeWidget: React.FC<ConciergeWidgetProps> = ({ onSelectVehicle }) => {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50">
      ...
      {/* Subtle, Discreet Floating Bubble Trigger Button */}
      {!isOpen && (
        <button
          onClick={() => setIsOpen(true)}
          aria-label="Open AI Car Concierge"
          className="group flex items-center gap-2.5 px-4 py-3 rounded-full bg-white border border-slate-200 shadow-xl hover:shadow-2xl text-slate-800 font-semibold text-xs transition-all duration-300 hover:scale-105 cursor-pointer"
        >
      ...
      {isOpen && (
        <div className="w-80 sm:w-96 bg-white border border-slate-200 rounded-3xl shadow-2xl overflow-hidden flex flex-col h-[500px] animate-in slide-in-from-bottom-8 duration-300">
          ...
          <div className="p-4 bg-slate-50 border-b border-slate-200 flex items-center justify-between">
            <div className="flex items-center gap-2.5">
              ...
              <div>
                <h3 className="text-xs font-bold text-slate-900 font-sans">AutoElite AI Concierge</h3>
                <p className="text-[10px] text-emerald-600 flex items-center gap-1 font-medium">
                  <span className="w-1.5 h-1.5 rounded-full bg-emerald-500" /> Online Assistant
                </p>
              </div>
            </div>
            <button
          ...
          <div className="p-4 flex-1 overflow-y-auto space-y-4 text-xs bg-white">
            {messages.map((msg) => (
              <div
                key={msg.id}
                  ...
                  <div className="w-full space-y-2 pt-1">
                    {msg.suggestedVehicles.map((v) => (
                      <div
                        ...
                        className="p-2.5 rounded-xl bg-slate-50 border border-slate-200 hover:border-slate-400 flex items-center gap-3 cursor-pointer transition-all group"
                      >
                        <img src={v.image} alt={v.model} className="w-14 h-10 rounded-lg object-cover shrink-0" />
                        <div className="flex-1 min-w-0">
                          <div className="text-[11px] font-bold text-slate-900 group-hover:text-slate-700 transition-colors truncate">
                            {v.make} {v.model}
                          </div>
                        ...
                        <ArrowRight className="w-3.5 h-3.5 text-slate-400 group-hover:text-slate-800 group-hover:translate-x-0.5 transition-all shrink-0" />
                      </div>
                    ))}
                  </div>
                )}
          ...
          {/* Prompt Chips Bar */}
          <div className="p-2.5 bg-slate-50 border-t border-slate-200/80 overflow-x-auto flex gap-1.5 scrollbar-none">
            {PRESET_PROMPTS.map((chip, idx) => (
              <button
                key={idx}
                onClick={() => handleSendPrompt(chip)}
                className="px-2.5 py-1 rounded-full bg-white border border-slate-200 text-[10px] text-slate-600 hover:text-slate-900 hover:bg-slate-100 whitespace-nowrap transition-colors shrink-0 cursor-pointer"
              >
                {chip}
              </button>
            ))}
          ...
          >
            <input
              ...
              className="flex-1 bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-xs text-slate-800 placeholder-slate-400 focus:outline-none focus:border-slate-400"
            />
            <button
              type="submit"
              aria-label="Send message to AI concierge"
              className="p-2 bg-slate-900 hover:bg-slate-800 rounded-xl text-white transition-colors cursor-pointer"
            >
